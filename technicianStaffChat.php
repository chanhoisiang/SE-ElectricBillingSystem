...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($username) && !empty($message)) {
        $stmt = $conn->prepare("INSERT INTO technician_staff_chat (username, sender, message) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $username, $sender, $message);

        if ($stmt->execute()) {
            header("Location: technicianStaffChat.php"); // Redirect back to chat page
            exit();
        } else {
...
$sql = "SELECT * FROM technician_staff_chat ORDER BY chat_time ASC";
...
<head>
    ...
    <style>
        ...
        .staff {
            background-color: rgb(185, 253, 216);
            align-self: flex-end;
        }
...
<body>
    ...
    <div class="chat-container">
        <h2>Technician Staff Chat</h2>
        <div class="chat-box">
            <?php while ($row = $result->fetch_assoc()) { ?>
                <p class="<?= $row['sender'] == 'staff' ? 'staff' : 'technician' ?>">
